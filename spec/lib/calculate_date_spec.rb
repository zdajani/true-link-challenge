...
RSpec.describe CalculateDate do
  ...
  describe 'next' do
    ...
    it 'calculates the next date' do
      recurrance_num = 1
      expect(subject.next(recurrance_num, start_date, 1)).to eq(Date.new(year, month, day))
    end
    ...
    it 'calculates the next 2 dates every 2 months' do
      ...
      expect(subject.next(recurrance_num, start_date, dates_num)).to eq(expected_dates)
    end

    it 'return the next date if start day is after current date' do
      recurrance_num = 1
      dates_num = 3
      start_day = Date.today.prev_week(:friday)
      expected_dates = [
